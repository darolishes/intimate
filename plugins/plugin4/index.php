...
function intimate_tales_plugin_4_enqueue_styles_scripts() {
    wp_enqueue_style('intimate-tales-plugin-4-styles', plugins_url('styles.css', __FILE__));
    wp_enqueue_script('intimate-tales-plugin-4-scripts', plugins_url('scripts.js', __FILE__), array('jquery'), '1.0.0', true);
}
add_action('wp_enqueue_scripts', 'intimate_tales_plugin_4_enqueue_styles_scripts');

// Load the ACF fields.
function intimate_tales_plugin_4_load_acf_fields() {
    $acf_fields = json_decode(file_get_contents(plugin_dir_path(__FILE__) . 'acf-fields.json'), true);
    foreach ($acf_fields as $field) {
        acf_add_local_field($field);
    }
}
add_action('acf/init', 'intimate_tales_plugin_4_load_acf_fields');
```